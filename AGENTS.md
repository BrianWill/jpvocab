...
Japanese vocabulary drilling browser-based app built with Go backend + web frontend. For now, assume a single user with no user login or need for security. The app has four top-level views:
...
- **Drill** � supports both the original round-based flashcard flow and an alternate **matching pairs mode**. In the standard mode, each round presents 10 words randomly drawn from the lexicon and the user marks each word as known or unknown; unknown words carry over to the next round alongside fresh picks. In matching pairs mode, the same round/pool progression is used, but the user matches Japanese words against shuffled meaning/POS cards. A word matched correctly on the first try counts as cleared; a word missed on its first attempt stays in the current round until eventually matched, then carries over into the next round instead of contributing to `doneCount`. The current in-progress drill mode and round state are persisted to SQLite so refreshing `/drill` or restarting the server restores the same state.
...
Manual backups are stored on disk under `backups/<backup-id>/` as JSON exports plus copied uploaded word images. Restoring a backup replaces the current user data tables and `static/images/words/`, with an optional "create safety backup first" step in the UI.
...
- **Matching pairs mode** — toggleable from both the drill restart modal and the shared settings modal. When enabled, `/drill` swaps the normal prompt/known/unknown layout for a two-column matching UI rendered by `drill-view.js`: Japanese word buttons on the left, shuffled meaning/POS cards on the right. Hovering a word still shows the drill tooltip with reading and kanji info. Matching mode uses the same filtered word pool and persisted session row as the standard drill, but stores extra matching-specific state in the session snapshot (`matchingRoundWords`, `matchingInfoWords`, `matchingMatchedPairs`, selected word id, redo/carryover ids, and first-try-correct ids).
- **Drill persistence** — drill sessions now save both answer events and durable UI state. `POST /api/drill/sessions/{id}/answers` records answers while also receiving the latest serialized state, and `PUT /api/drill/sessions/{id}/state` persists state-only updates such as matching-pairs selections and other non-answer progress.
...
# The bundled dictionary DB is now a slim runtime artifact.
# Local rebuild of flattened lookup tables from dict/jdict.db is intentionally unsupported.
# If dict/jdict.db changes, refresh dict/jdict.db.gz from it:
gzip -c dict/jdict.db > dict/jdict.db.gz
...
- **Add words flow** � the user pastes Japanese words into an "add words" modal; the backend streams results back via SSE, adding words one by one and displaying them in the add-result modal. Implemented:
  ...
  - Initial word info now comes from the bundled local runtime dictionary (`dict/jdict.db`) when those fields are not already provided by a higher-priority source
  - Runtime dictionary word lookup reads from a precomputed `dict_word_lookup` table in `dict/jdict.db`
  - Runtime kanji lookup reads from a precomputed `dict_kanji_lookup` table in `dict/jdict.db`, which stores the shared meanings list and shared readings list for each kanji literal
...
- **Audio** — word and sentence playback use on-the-fly VoiceVox synthesis when VoiceVox is available at `http://localhost:50021`. Shared playback helpers call `POST /api/voicevox/synthesize` and play the returned audio immediately rather than reading generated files from disk. If VoiceVox is unavailable or synthesis fails, playback falls back to the browser Web Speech API TTS.
...
- **Generate translation** — the story detail page has a "Generate translation" button that opens a confirmation modal with an AI provider selector (same provider/model list as the lexicon add-edit modal). On confirm, calls `POST /api/stories/{id}/generate-translation` with `{ai_model, chunk_position}`. The backend sends only the target sentences as a plain ordered string array, and the AI returns a matching array of literal English sentence translations. Translations are stored in `story_sentences.english_text`. Response is NDJSON: `{status, sentenceCount}` immediately, then `{allDone:true}` on success. Literal translation over natural English is intentional — sentences are translated individually with no cross-sentence context, which is the desired behaviour for language learning. Story translation does not request AI-generated word info.
...
`src/` is a standalone Go module (separate `go.mod`) that runs a SQLite-backed HTTP server on port **49200** (dynamic/private range). Open `http://localhost:49200/welcome` in a browser to use the app.

- **`main.go`** \u{FFFD} entry point; opens the DB and runs the web server on the main goroutine.
...
- **`dict.go` / `dict_lookup.go`** � local bundled dictionary support. `dict.go` handles finding/opening the read-only SQLite dictionary and background decompression of `jdict.db.gz`; `dict_lookup.go` is a thin cached wrapper around `internal/dictlookup`, which requires the precomputed `dict_word_lookup` / `dict_kanji_lookup` tables at runtime.
- **`internal/dictlookup/`** — shared runtime dictionary lookup logic. `LookupWordInDB` and `LookupKanjiInDB` read only from the precomputed flattened lookup tables stored in `dict/jdict.db`.
- **`cmd/build-dict-lookup/`** — intentionally fails with a clear message because the slim runtime `dict/jdict.db` no longer contains the normalized JMdict/Kanjidic source tables needed to rebuild the flattened lookup tables locally.
...
- **`db_settings.go`** � user settings: `getDrillSettings` and `putDrillSettings` read/write the `user_settings` table using key/value pairs. `drillSettings` always returns fully-populated values with no null fields � `MaxWords` defaults to `100`, `RoundSize` to `10`, `WordTypes` to all four types, `NewWordTarget` to `8`, `SkipAnswerReveal` to `false`, and `MatchingPairsMode` to `false`. `MaxWords` is always = 1; `0` is not a valid value. The frontend should treat the `GET /api/settings/drill` response as always having concrete values and needs no null-handling.
- **`backup.go`** — file-based backup/restore system. `createBackup` exports the user-data tables to `backups/<id>/data.json`, writes `manifest.json` with counts/format metadata, and copies any referenced word images into the same backup directory. `restoreBackup` validates the manifest/data/images, optionally creates a safety backup first, clears the current user-data tables, re-inserts rows in dependency-safe order, and replaces `static/images/words/` with the backed-up image set. Also defines the `/api/backups`, `/api/backups/{id}/restore` handlers.
...
- **`db_stories.go`** also contains `deleteStory`, which removes a story and its sentence rows.
...
- **`routes_stories.go`** — story API handlers: `GET /api/stories`, `GET /api/stories/{id}`, `POST /api/stories/{id}/noted-words`, `DELETE /api/stories/{id}/noted-words`, `POST /api/stories/{id}/generate-translation` (AI sentence translation only, NDJSON streaming). Long-running story AI routes share the `runStoryNDJSONJob` helper for the initial status event, heartbeat updates, and final token-usage payload.
...
- **`ai.go`** - Shared AI types, prompts, few-shot examples, and the shared `aiModelTarget` provider/model adapter used by `autoFillWord`, `autoFillWordsBatch`, `translateStory`, tutor chat, and image-suggestion helpers. `autoFillWordsBatch` splits words into chunks of `autoFillBatchSize` (20) and runs chunks concurrently, each as a single AI call returning a JSON array. `translateStory` sends only the target story sentences in one call and returns ordered translation strings. No direct DB access. Environment variables: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_API_KEY`, `MISTRAL_API_KEY`, `GLM_API_KEY`.
...
- **Bundled dictionary assets** � the repo includes `dict/jdict.db`, `dict/jdict.db.gz`, and `dict/strip.js`. `jdict.db` is now a slim runtime artifact that contains only the precomputed `dict_word_lookup` and `dict_kanji_lookup` tables used at runtime; it no longer contains the normalized JMdict/Kanjidic source tables. If `jdict.db` changes, regenerate `dict/jdict.db.gz` in the same change. Runtime lookups are local SQLite reads only; no external API is involved.
- **`templates/`** - HTML templates parsed from disk on every request (live-editable without restart). `base.html` is the admin shell; `app_nav.html` is the shared top-nav partial used by the main app pages and includes the page links and the `?` app-logo link back to `/welcome`.
...
| `POST` | `/api/voicevox/preview` | Generate a short WAV preview clip for the current VoiceVox settings |
| `POST` | `/api/voicevox/synthesize` | Generate on-demand OGG audio for Japanese playback |
| `GET` | `/api/drill/sessions/current` | Return the current in-progress drill session, if one exists |
| `PUT` | `/api/drill/sessions/{id}/state` | Persist the latest serialized drill UI/session state without recording an answer |
| `POST` | `/api/stories/{id}/generate-translation` | AI-generate English sentence translations for a story or chunk; body: `{ai_model, chunk_position}`; streams NDJSON |
...
| `GET` | `/api/backups` | List available on-disk backups with counts and timestamps |
| `POST` | `/api/backups` | Create a full backup of user data tables plus uploaded word images |
| `POST` | `/api/backups/{id}/restore` | Restore a backup, optionally creating a safety backup first |
...
- **`words`** — the lexicon; one row per word with reading, part of speech, meaning, example sentences, an optional `image_path` (relative to `static/`, e.g. `images/words/食べる.jpg`), drill counts, target, timestamps, a `kanji_data` JSON column (array of `{id, reading}` linking to the `kanji` table), and `tracked INTEGER NOT NULL DEFAULT 1`. `word` column has a unique index. Because `word` is unique, uploaded image files are named after the word text itself. `tracked=1` means the user has explicitly added the word; `tracked=0` means the word was auto-inserted from story content and hasn't been explicitly added yet.
- **`kanji`** � one row per kanji character with `character`, `meanings` (JSON array of English meanings), and `readings` (JSON array of shared on/kun display readings). Word-specific reading segments are still stored per-word in the `kanji_data` column of `words`. Served via `/api/kanji`.
- **`drill_sessions`** � one row per drill session with a `started_at` timestamp, a persisted durable UI/session snapshot in `state_json` (round/pool/redo/remaining/sidebar/last-answered state for the standard mode, plus matching-pairs state such as `matchingRoundWords`, `matchingInfoWords`, selected/matched/carryover ids, etc.), and `completed_at` for distinguishing the single active in-progress drill from completed sessions.
- **`drill_answers`** � one row per answer within a session; references `words` and `drill_sessions`; stores `correct` (0/1) and `answered_at`.
- **`user_settings`** � key/value store for user preferences. Current keys: `drill_max_words` (int, always = 1; absent from the table means the default of `100` is used), `drill_round_size` (int), `drill_word_types` (JSON string array), `drill_new_word_target` (int), `drill_skip_answer_reveal` (bool), and `drill_matching_pairs_mode` (bool).
- **`stories`** - one row per story with required `title`, `story_words_json` (JSON array of unique base word strings for the story — used as the word-info lookup key against the `words` table at query time), `noted_words_json` JSON (an ordered array of `{displayWord, baseWord, english?, createdAt}` for the story's noted words), and `created_at` timestamp.
...
Backups are not stored in SQLite. They live in the repo-root `backups/` directory, one subdirectory per backup id, and currently export `kanji`, `words`, `drill_sessions`, `drill_answers`, `user_settings`, `stories`, `story_sentences`, `tutor_prompts`, `activity_events`, and `token_usage`, plus copied `static/images/words/` assets referenced by `words.image_path`.
