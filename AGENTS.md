...
Table definitions live in the `migrate()` function in `db_schema.go`. Schema is versioned via `PRAGMA user_version` � each entry in the migrations slice runs exactly once. Current schema version: **9**. Current tables:
