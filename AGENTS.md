...
- **`routes_stories.go`** — story API handlers: `GET /api/stories`, `GET /api/stories/{id}`, `POST /api/stories/{id}/noted-words`, `DELETE /api/stories/{id}/noted-words`, `POST /api/stories/{id}/generate-audio` (VoiceVox, NDJSON streaming), `POST /api/stories/{id}/generate-translation` (AI sentence translation only, NDJSON streaming). Long-running story AI routes share the `runStoryNDJSONJob` helper for the initial status event, heartbeat updates, and final token-usage payload.
