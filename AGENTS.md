...
# Rebuild the bundled flattened dictionary lookup tables, then refresh dict/jdict.db.gz
cd src && go run ./cmd/build-dict-lookup && cd .. && gzip -c dict/jdict.db > dict/jdict.db.gz
...
- **Add words flow** � the user pastes Japanese words into an "add words" modal; the backend streams results back via SSE, adding words one by one and displaying them in the add-result modal. Implemented:
  ...
  - Runtime dictionary word lookup reads from a precomputed `dict_word_lookup` table in `dict/jdict.db` so bulk word adds do one simple keyed lookup instead of reconstructing readings/glosses/kanji info across multiple normalized JMdict/Kanjidic queries
  - Runtime kanji lookup now also reads from a precomputed `dict_kanji_lookup` table in `dict/jdict.db`, which stores the shared meanings list and shared readings list for each kanji literal
...
- **`dict.go` / `dict_lookup.go`** � local bundled dictionary support. `dict.go` handles finding/opening the read-only SQLite dictionary and background decompression of `jdict.db.gz`; `dict_lookup.go` is now a thin cached wrapper around `internal/dictlookup`, which prefers the precomputed `dict_word_lookup` / `dict_kanji_lookup` tables and falls back to normalized JMdict/Kanjidic tables if needed.
- **`internal/dictlookup/`** — shared dictionary lookup/build logic. `LookupWordInDB` and `LookupKanjiInDB` serve runtime lookups; `RebuildLookupTable` regenerates both the precomputed `dict_word_lookup` and `dict_kanji_lookup` tables inside `dict/jdict.db`.
- **`cmd/build-dict-lookup/`** — command used to rebuild `dict/jdict.db`'s `dict_word_lookup` and `dict_kanji_lookup` tables before recompressing `dict/jdict.db.gz`.
...
- **Bundled dictionary assets** � the repo includes `dict/jdict.db`, `dict/jdict.db.gz`, and `dict/strip.js`. `jdict.db` now contains both the normalized JMdict/Kanjidic tables and the precomputed `dict_word_lookup` and `dict_kanji_lookup` tables used at runtime. If `jdict.db` changes, regenerate `dict/jdict.db.gz` in the same change. Runtime lookups are local SQLite reads only; no external API is involved.
