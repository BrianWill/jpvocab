...
- **`static/images/words/`** — word images served statically. Files are named after the word text (e.g. `猫.jpg`, `天気.jpg`). The `image_path` column stores the path relative to `static/`; the frontend constructs the full URL as `/static/<image_path>`. Images are optional — words without images have `NULL` in `image_path`. Images are displayed in the lexicon table as a fixed-width column on the far left; the image cell uses `rowspan="2"` to span both the main and example rows, with `object-fit: cover` and `vertical-align: middle`.
