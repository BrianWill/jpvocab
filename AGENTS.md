...
Table definitions live in the `migrate()` function in `db_schema.go`. Schema is versioned via `PRAGMA user_version` � each entry in the migrations slice runs exactly once. Current schema version: **11**. Current tables:

- **`words`** � the lexicon; one row per word with reading, part of speech, meaning, example sentences, audio flags (`has_word_audio`, `has_sentence_audio` INTEGER booleans — paths are derived as `static/audio/{word}.wav` / `static/audio/{word}_sentence.wav`, not stored), an optional `image_path` (relative to `static/`, e.g. `images/words/食べる.jpg`), drill counts, target, timestamps, a `kanji_data` JSON column (array of `{id, reading}` linking to the `kanji` table), and `in_lexicon INTEGER NOT NULL DEFAULT 1`. `word` column has a unique index. Because `word` is unique, image and audio files are named after the word text itself. `in_lexicon=1` means the user has explicitly added the word; `in_lexicon=0` will be used for words auto-inserted from story content that haven't been explicitly added yet.
