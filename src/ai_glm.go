...
// the content of the first choice. The API key is read from GLM_API_KEY.
func callGLM(model string, messages []message) (string, error) {
	apiKey := os.Getenv("GLM_API_KEY")
	if apiKey == "" {
		return "", fmt.Errorf("GLM_API_KEY environment variable is not set")
	}
...
func autoFillWordGLM(word, model string) (*wordAutoFill, error) {
...
func autoFillWordsBatchGLM(words []string, model string) ([]*wordAutoFill, error) {
...
func rerollMeaningGLM(word, currentMeaning, model string) ([]string, error) {
	messages := []message{
...
func rerollExamplesGLM(word, model string) ([]examplePair, error) {
	messages := []message{
