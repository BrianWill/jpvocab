...
- **Stories** - a top-level page at `/stories` that currently lists available stories by title and date. Each entry links to `/stories/{id}`, which renders the story text by reconstructing each sentence from the stored word-token display text. On the detail page, the header still includes the Stories link, but it is intentionally not marked as the active nav item.
  ...
  The detail page also has a collapsible left "Noted Words" sidebar inside the scroll area. Hovering a non-punctuation word token shows a tooltip hint; clicking the token saves that token's base word into the story's persisted noted-words list; the sidebar lists noted words with remove buttons and a placeholder "Add all to Lexicon" button that is intentionally non-functional for now.
