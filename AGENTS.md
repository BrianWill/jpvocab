...
- **`tutor_prompts`** — one row per tutor mode; `label` (display name), `system_prompt` (mode-specific instructions, without the shared JSON format prefix which is prepended at runtime), `greeting` (JSON string used as the opening message; special sentinels `__random_free__` and `__random_free_en__` trigger random topic greetings in the frontend), `jp_input` (1 if mode expects Japanese input — controls STT language), `can_remove` (0 for built-in seeded prompts, 1 for user-created custom prompts). Seeded by `seedTutorPrompts()` in `db_schema.go` whenever the table is empty.
