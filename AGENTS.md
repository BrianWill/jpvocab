...
# Rebuild the bundled flattened dictionary lookup table, then refresh dict/jdict.db.gz
...
- **`dict.go` / `dict_lookup.go`** � local bundled dictionary support. `dict.go` handles finding/opening the read-only SQLite dictionary and background decompression of `jdict.db.gz`; `dict_lookup.go` is now a thin cached wrapper around `internal/dictlookup`, which prefers the precomputed `dict_word_lookup` table and falls back to normalized JMdict/Kanjidic tables if needed.
- **`internal/dictlookup/`** — shared dictionary lookup/build logic. `LookupWordInDB` serves runtime word lookups; `RebuildLookupTable` regenerates the precomputed `dict_word_lookup` table inside `dict/jdict.db`.
- **`cmd/build-dict-lookup/`** — command used to rebuild `dict/jdict.db`'s `dict_word_lookup` table before recompressing `dict/jdict.db.gz`.
...
- **Bundled dictionary assets** � the repo includes `dict/jdict.db`, `dict/jdict.db.gz`, and `dict/strip.js`. `jdict.db` now contains both the normalized JMdict/Kanjidic tables and the precomputed `dict_word_lookup` table used at runtime. If `jdict.db` changes, regenerate `dict/jdict.db.gz` in the same change. Runtime lookups are local SQLite reads only; no external API is involved.
