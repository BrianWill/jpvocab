...
| `GET` | `/api/stories/{id}` | Return one story by id for the story detail page |
...
Table definitions live in the `migrate()` function in `db_schema.go`. Schema is versioned via `PRAGMA user_version` � each entry in the migrations slice runs exactly once. Current schema version: **7**. Current tables:
...
- **`stories`** - one row per story with required `title`, optional narration `audio_path`, and `created_at` timestamp.
